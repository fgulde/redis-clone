...
void TransactionDispatcher::dispatch(const RespValue& request, const Command& cmd,
  ...
  // MULTI and EXEC are always handled directly by their command implementations,
  // even when a transaction is active (MULTI nested error, EXEC execution).
  if (cmd.type == Command::Type::Multi || cmd.type == Command::Type::Exec) {
