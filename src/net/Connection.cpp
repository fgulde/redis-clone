...
Connection::Connection(tcp::socket socket, Store &store, BlockingManager &blocking_manager, WatchManager &watch_manager,
  ...
  , client_handler_(store, blocking_manager, watch_manager, config, std::move(get_connected_clients), std::move(get_used_memory), CommandHandler::RegistryKind::Client)
  , replication_handler_(store, blocking_manager, watch_manager, config, {}, {}, CommandHandler::RegistryKind::Replication) {}
